...
@JsonSerialize(using = Log.Serializer.class)
public class Log implements Serializable {
    public static class Serializer extends JsonSerializer<Log> {
        @Override
        public void serialize(Log log, JsonGenerator jgen, SerializerProvider provider) throws IOException {
            jgen.writeStartObject();
            jgen.writeNumberField("log_timestamp", log.getTimestamp());
            jgen.writeStringField("text", log.getMessage());
            jgen.writeStringField("lint_source", log.getSource());
            for (String key : log.getAnnotations().keySet()) {
                jgen.writeStringField(key, log.getAnnotations().get(key));
            }
            jgen.writeEndObject();
        }
    }
